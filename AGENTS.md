...
- **`MPS_KNOWLEDGE_BASE`** — a hardcoded array (~227 entries) digitized from two internal Mitsui Petrochemical engineering documents (`MPS-07011` for TA UNIT, `MPS-07012` for PTA UNIT — "Control on Process Parameters and Countermeasures against Process Deviation"). Each entry holds an equipment/factor's Normal/Allowable/Critical figures, the engineering rationale, and ordered operator countermeasure steps, keyed by instrument tag (e.g. `TICSK-2601`). `normalizeInstrumentKey`/`findKbEntry` (defined alongside it, before `SMART_AGENT`) match a record's `tagNo` against the KB by exact string or by a normalized ISA-style key (first letter + loop number), falling back to `null` on ambiguous matches rather than guessing. This is **read-only reference data — it does not feed `STATE._deriveAbnormal`** and never changes `isAbnormal`; it's consulted only to render the standards panel in `openActionModal` (`APP.renderKbPanel`), a badge on abnormal cards, and to enrich `SMART_AGENT`'s auto-draft text.
- **`SMART_AGENT`** — generates Thai-language natural-language summaries/analysis text for abnormal readings and shift summaries (rule-based, not an external AI call). `analyze` takes an optional 5th `kbEntry` argument (from `findKbEntry`) and appends its countermeasure steps to the generic text when present; omitted, behavior is unchanged.
