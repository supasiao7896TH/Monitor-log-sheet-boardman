...
- **Equipment-specific countermeasure recommendations, digitized from internal MPS standards (Phase 1 of PI Data Link integration).** The plant runs a PI Data Link/PI Server historian, and the eventual goal is for this app to pull readings from it directly instead of via Excel export (see below). Before building that real-time integration, the user asked to digitize two internal Mitsui Petrochemical engineering documents already in this repo (`MPS-07011` for TA UNIT, `MPS-07012` for PTA UNIT — "Control on Process Parameters and Countermeasures against Process Deviation") into the app, so abnormal-reading cards can show genuinely equipment-specific guidance (Normal/Allowable/Critical figures, the engineering rationale, and numbered operator actions) instead of the old `SMART_AGENT` canned high/low/deviation sentence. Cross-checking confirmed the documents' instrument tags match the plant's real PI tags almost exactly (e.g. `TC-2601` ↔ `TICSK-2601`, `LC-2201` ↔ `LICK-2201`), so the same digitized data (and the tag-normalization matching logic built to reconcile the naming variants) will double as the tag-mapping table needed for the PI Web API phase. This phase was deliberately scoped **additive-only**: the new `MPS_KNOWLEDGE_BASE` data and `findKbEntry` matching are consulted purely for display (a card badge, a standards panel in the record modal, and richer Smart-Assist text) and never touch `STATE._deriveAbnormal` or the existing min/max abnormal-flagging logic — keeping the change low-risk since it's pure UI/reference-data addition. Real-time PI Web API integration and any true 3-tier (Normal/Allowable/Critical) severity logic were explicitly deferred to a later phase.
